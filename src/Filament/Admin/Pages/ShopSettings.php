            ...
            ->components([
                ...
                Section::make(Theme::trans('shop.section_landing'))
                    ...
                    ->schema([
                        ...
                        /*
                         * And beside those for the same reason: it is a
                         * decision about how much of the shop a customer
                         * drives. Off by default, because it changes what an
                         * unpaid bill does - one invoice is one debt, so an
                         * unpaid one stops every service on it.
                         */
                        /*
                         * Where the money is decided, not where the shop's
                         * behaviour is: it changes what a number on a package
                         * means rather than what a customer may do.
                         */
                        Toggle::make('shop_tax_inclusive')
                            ->label(Theme::trans('shop.tax_inclusive'))
                            ->helperText(Theme::trans('shop.tax_inclusive_helper'))
                            ->inline(false)
                            ->disabled(!$may),

                        Toggle::make('shop_vat_check')
                            ->label(Theme::trans('shop.vat_check'))
                            ->helperText(Theme::trans('shop.vat_check_helper'))
                            ->inline(false)
                            ->disabled(!$may),

                        Toggle::make('shop_basket')
                            ->label(Theme::trans('shop.basket'))
                            ->helperText(Theme::trans('shop.basket_helper'))
                            ->inline(false)
                            ->disabled(!$may),
                    ])
                    ->columns(['default' => 1]),
                 ...
                 * Who is selling.
                 *
                 * Its own section rather than a few fields among the money
                 * ones, because it is answering a different question: not what
                 * things cost, but who is asking for the money. An invoice
                 * without these is not an invoice in the Netherlands, and the
                 * shop printed the panel's name and nothing else for its whole
                 * life until now.
                 */
                Section::make(Theme::trans('shop.section_company'))
                    ->description(Theme::trans('shop.section_company_helper'))
                    ->columns(['default' => 1, 'sm' => 2])
                    ->schema([
                        TextInput::make('shop_company_name')
                            ->label(Theme::trans('shop.company_name'))
                            ->helperText(Theme::trans('shop.company_name_helper'))
                            ->maxLength(120)
                            ->disabled(!$may),

                        TextInput::make('shop_company_email')
                            ->label(Theme::trans('shop.company_email'))
                            ->email()
                            ->maxLength(191)
                            ->disabled(!$may),

                        Textarea::make('shop_company_address')
                            ->label(Theme::trans('shop.company_address'))
                            ->helperText(Theme::trans('shop.company_address_helper'))
                            ->rows(4)
                            ->maxLength(600)
                            ->disabled(!$may)
                            ->columnSpanFull(),

                        TextInput::make('shop_company_vat')
                            ->label(Theme::trans('shop.company_vat'))
                            ->helperText(Theme::trans('shop.company_vat_helper'))
                            ->maxLength(32)
                            ->disabled(!$may),

                        TextInput::make('shop_company_coc')
                            ->label(Theme::trans('shop.company_coc'))
                            ->maxLength(32)
                            ->disabled(!$may),

                        TextInput::make('shop_company_country')
                            ->label(Theme::trans('shop.company_country'))
                            ->helperText(Theme::trans('shop.company_country_helper'))
                            ->maxLength(2)
                            ->disabled(!$may),

                        TextInput::make('shop_due_days')
                            ->label(Theme::trans('shop.due_days'))
                            ->helperText(Theme::trans('shop.due_days_helper'))
                            ->numeric()
                            ->minValue(0)
                            ->maxValue(90)
                            ->disabled(!$may),
