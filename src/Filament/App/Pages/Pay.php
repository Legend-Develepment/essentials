...
class Pay extends Page implements HasActions, HasSchemas
{
    ...
    public function summary(): ?array
    {
        ...
        return [
            ...
            /*
             * What the balance already covered, and what is therefore left.
             *
             * Both, and never one of them. A page that quietly showed a smaller
             * total would leave the customer working out where the rest went;
             * one that showed only the total would ask them for money the shop
             * is already holding.
             */
            'credit' => (int) $invoice->credit > 0
                ? Money::format((int) $invoice->credit, $currency)
                : null,
            'payable' => Money::format($invoice->due(), $currency),
