...
class Mollie implements Gateway
{
    ...
    public function start(Invoice $invoice, string $returnUrl, string $webhookUrl): ?string
    {
        ...
        if (Gateways::remember($invoice, self::KEY, $id, Payment::OPEN, is_array($body) ? $this->trim($body) : null) === null) {
            report(new RuntimeException(
                'The payment could not be recorded, so nobody was sent to Mollie. The reason is the exception above this one.',
            ));
    ...
    /**
     * Mollie refunds against the payment itself, which is what we hold.
     *
     * Partial refunds are ordinary here: the amount is simply less than the
     * payment was. A refund of a payment Mollie has not settled yet is refused
     * by them, which is correct, and is why nothing here tries to guess.
     */
    public function refund(Payment $payment, int $amount): bool
    {
        ...
        try {
            $response = Http::withToken($key)
                ...
                ->acceptJson()
                ->asJson()
                ->post(self::API . '/' . rawurlencode((string) $payment->gateway_id) . '/refunds', [
                    'amount' => [
                        'currency' => Money::currency($payment->currency),
                        'value' => Money::decimal($amount),
                    ],
                    'description' => mb_substr(Theme::trans('credit.refund_description', [
                        'number' => (string) ($payment->invoice?->number ?? ''),
                    ]), 0, 140),
                ]);
        } catch (Throwable $exception) {
            report($exception);
        ...
        if (!$response->successful()) {
            report(new RuntimeException(
                'Mollie refused a refund on payment ' . $payment->gateway_id . ': HTTP ' . $response->status(),
            ));
