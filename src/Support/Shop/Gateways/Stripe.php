...
class Stripe implements Gateway
{
    ...
    public function start(Invoice $invoice, string $returnUrl, string $webhookUrl): ?string
    {
        ...
        try {
            $response = Http::withToken($secret)
                ...
                ->post(self::API, [
                    ...
                    'line_items[0][price_data][unit_amount]' => $invoice->due(),
        ...
        if (Gateways::remember($invoice, self::KEY, $id, Payment::OPEN, is_array($body) ? $this->trim($body) : null) === null) {
            report(new RuntimeException(
                'The payment could not be recorded, so nobody was sent to Stripe. The reason is the exception above this one.',
            ));
    ...
    /**
     * Stripe refunds a payment intent, and what we hold is a checkout session.
     *
     * So the session is read first for the intent it produced. Keeping that
     * intent in `raw` at settle time would save this call, but only for
     * payments taken after the change - and a refund is most often wanted for
     * an older one.
     */
    public function refund(Payment $payment, int $amount): bool
    {
        ...
        try {
            ...
            $intent = (string) ($session->json('payment_intent') ?? '');

            if ($intent === '') {
                report(new RuntimeException(
                    'Stripe session ' . $payment->gateway_id . ' has no payment intent to refund.',
                ));
        ...
        if (!$response->successful()) {
            report(new RuntimeException(
                'Stripe refused a refund on ' . $payment->gateway_id . ': HTTP ' . $response->status()
