...
class Schedule
{
    public static function register(Scheduler $schedule): void
    {
        try {
            ...
            /*
             * And whoever is waiting for something that has stock again.
             *
             * Its own entry, five minutes apart rather than nightly, because
             * what it catches is a place coming free rather than a date
             * arriving: somebody who cancels at noon frees a place at noon, and
             * telling the next person about it the following morning is telling
             * them after it has gone again.
             *
             * Cheap when there is nothing to do, which is the ordinary case:
             * one query that usually returns no rows at all. Registered only
             * when the waiting list is switched on and its table exists, so a
             * panel without one has no entry rather than one that wakes to
             * decide it has nothing to do.
             */
            if (Waiting::ready()) {
                $schedule
                    ->call(static fn () => Waiting::release())
                    ->name('legend-theme:waitlist')
