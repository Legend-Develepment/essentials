...
class Orders
{
    ...
    /**
     * An order that belongs to this person, or null.
     *
     * Theirs and only theirs. A page left open in a tab has no claim on an id
     * somebody typed into it, and the same sentence was written out four times
     * across three pages - which is three chances for one of them to be written
     * slightly differently.
     *
     * The person is a parameter rather than read from user() in here, so
     * nothing that arrives without a session - the API, a queued job - can be
     * handed a caller it did not mean.
     *
     * @param  array<int, string>  $with
     */
    public static function mine(int $id, int $userId, array $with = []): ?Order
    {
        if ($id <= 0 || $userId <= 0) {
            return null;
        }
        ...
        return $row instanceof Order && (int) $row->user_id === $userId ? $row : null;
    }
    ...
    {
        /*
         * Not "cancel every unpaid invoice this order is on" any more.
         *
         * An invoice can bill two services now, and withdrawing the whole
         * document because one of them was cancelled would hand the customer a
         * free month of the one they kept. Invoices::revise() takes the line
         * off and recomputes what is left, and withdraws the invoice only when
         * that leaves nothing - which is every invoice that ever billed one
         * order, so the old behaviour is still the behaviour in the old case.
         */
        Invoices::revise($order);
    }
