...
return [
    'shrunk' => 'A picture was made smaller',
    'shrunk_body' => ':name arrived at :was and is drawn far smaller than that, so a :now copy was stored in its place. The one you chose is untouched on your own machine.',
    ...
    'groups' => [
        ...
        'valheim' => 'Valheim',
        'languages' => 'Languages',
        'files_where' => 'Where files are kept',
        'files_bucket' => 'The bucket',
        'files_cdn' => 'The CDN',
        'files_mirror' => 'Languages, kept off the panel',
     ...
     *
     * The words are about a destination rather than about a provider, because
     * the same three sentences are true of a bucket and of a CDN and an
     * administrator setting one up does not care which of them they are looking
     * at until the fields differ.
     */
    'files' => [
        'where' => 'Files are kept',
        'where_helper' => 'On the panel they sit on its own disk, which is where they have always gone and needs nothing set up. Anywhere else is somewhere this panel does not have to hold them, and is served nearer to whoever is looking. A destination that will not answer falls back to the panel rather than losing an upload.',
        'panel' => 'On this panel',
        's3' => 'In a bucket (S3, R2, MinIO, Wasabi)',
        'cdn' => 'On a CDN',
        'read_from' => 'Read from',
        'read_from_helper' => 'Where a file is fetched from, which is not always where it was written. A CDN in front of a bucket goes here, and so does a delivery address that differs from the one the API is on. Empty lets the destination work it out itself.',
        ...
        'cdn_title' => 'The CDN',
        'cdn_helper' => 'A CDN speaking the Modora API. The token is a server-to-server one and it is full admin on that account, so it is kept out of an exported settings file the way every other credential here is.',
        'cdn_base' => 'Address',
        'cdn_base_helper' => 'Where the API lives. If files are served from somewhere else, put that address in Read from above.',
        'cdn_token' => 'Token',
        'cdn_token_helper' => 'Sent as X-Internal-Token. Anyone holding it can write to and delete from the whole account.',
        'cdn_folder' => 'Folder',
        'cdn_folder_helper' => 'A folder under the account to keep this panel’s files in, so one CDN can serve several panels without them treading on each other.',

        'move' => 'Move what is still on the panel',
        'move_confirm' => 'The sidebar icon, the panel backdrop and the sign-in backdrop are copied to the destination and their addresses rewritten. The copies on this panel are left where they are, so nothing breaks if you change your mind. Pictures uploaded from here on go to the destination anyway; this is only for the ones already here.',
        'move_done' => 'Moved',
        'move_done_body' => 'Looked at :looked, moved :moved, could not move :failed.',
        'check' => 'Test this',
        'check_ok' => 'It works',
        'check_ok_body' => 'A file was written, fetched back over its public address and removed again.',
        'check_bad' => 'That did not work',
        'check_panel' => 'Files are set to be kept on this panel, so there is nothing to test.',
        'check_refused' => 'The destination refused the file and said nothing about why.',
        'check_unreadable' => 'It took the file, but it could not be read back from :url. That is the address a browser will use, so a file nobody can fetch is a broken picture later. Check Read from, and that the destination serves files publicly.',
        'bucket_missing' => 'The key, the secret and the bucket name are all needed before there is anything to test.',
        'cdn_missing' => 'The address and the token are both needed before there is anything to test.',
        'cdn_shape' => 'It accepted the file and then answered in a shape this panel could not find an address in. What it said was: :body',
        'mirror_minutes' => 'Look for changed languages every',
        'mirror_minutes_helper' => 'In minutes. Looking is cheap: each uploaded language is read, hashed and compared with what was last sent, so an ordinary pass sends nothing at all. Only a language somebody has changed crosses the wire.',
        'mirror_now' => 'Copy languages now',
        'mirror_done' => 'Languages copied',
        'mirror_done_body' => 'Looked at :looked, sent :sent, could not send :failed.',
        'mirror_restore' => 'Restore languages',
        'mirror_restore_confirm' => 'This writes every language in the off-panel copy over what is on this panel. That is the point of it after an upgrade, and there is no way to remove an installed language afterwards, so it is worth being sure.',
        'mirror_back' => 'Languages restored',
        'mirror_back_body' => 'Found :found, put back :put, could not fetch :failed.',
    ],
    ...
    'pages' => [
        ...
        'files' => 'Storage and CDN',
        'files_helper' => 'Where the files this plugin keeps are put, and the address they are read from.',
    ],
    ...
    'features' => [
        ...
        'files' => 'Storage and CDN',
        'files_helper' => 'Keeping this plugin’s files somewhere other than the panel: an S3 bucket, or a CDN. Off is not “no files” - it is the panel’s own disk, which is where they have always gone. What this decides is whether anywhere else is offered at all. A destination that will not answer falls back to the panel rather than losing an upload, and an address already written down is never taken back: changing this decides where the next file goes, not where the last one lives.',
        'minecraft' => 'Minecraft',
        ...
        'updating' => 'Updating notice',
        'waitlist' => 'Waiting list',
        'waitlist_helper' => 'Letting somebody ask to be told when a sold-out package is for sale again. When stock returns everybody waiting for that package is told at once and it goes to whoever buys first - nothing is held for anybody, and every message says so. Being told takes them off the list, so one asking buys one notification and never a standing subscription. It needs the shop, and it is the one thing in the shop that writes to a customer who has not bought anything.',
        'updating_helper' => 'One line at the top of the page while this plugin installs an update, and for five minutes after it finishes. It cannot be shown during the update itself - while the release is being swapped in, Pelican reads this plugin as not installed and loads none of it, so there is nothing of ours left to draw with. What it is for is the person who met a half-drawn page, waited, and came back: the line tells them what they saw.',
        ...
        'credit' => 'Credit and refunds',
        'credit_helper' => 'Money the shop holds for a customer. A refund can go back to the card it came from or stay on the account as credit; either way a credit note is written, and credit on an account comes off the next invoice automatically before the customer is ever asked to pay. Its own right, because marking an invoice paid records that money arrived and this hands money out.',
        'upgrades' => 'Upgrade and downgrade',
        'upgrades_helper' => 'Moving a live service to another package without buying a new one. What is left of the period already paid for comes back, the same stretch is charged at the new price, and the difference is invoiced or put on the customer’s account. Each package lists which others it may be moved to, and only ones sharing its egg are offered: a different egg is a different server, not a bigger one.',
        'addons' => 'Extras',
        'addons_helper' => 'Things sold alongside a package: more memory, another backup slot, or something that is only a line on the invoice. Each one says which packages it fits and what it adds to the server, and it is charged either with every renewal or once. Bought at checkout or later on a running service, where it is pro-rated to what is left of the period. Its own right, because what an extra may add to somebody’s server is a decision about their machine rather than about a price list.',
        'tickets' => 'Tickets',
        'tickets_helper' => 'Somewhere for customers to ask a question from inside the panel, beside the service they are asking about - which is the one thing a chat channel cannot do. Answered on a page here, or handed to Discord through Modora, whichever the Tickets page is set to. Every question and every answer is kept in this panel either way, so nothing is lost when the far end cannot be reached. Its own right, because answering customers is a job somebody is given rather than one that comes with pricing packages.',
    ...
    'channel' => [
        ...
        'token_helper' => 'The dev channel is published from a private repository, so reading it needs a GitHub token - a fine-grained personal access token with read access to that repository\'s contents, and nothing more. Stable and beta are public and need none. It stays on this panel: it is not written to an exported settings file.',
        'stable' => 'Stable',
        'beta' => 'Beta',
    ...
    'languages' => [
        ...
        'remove' => 'Remove a language',
        'remove_which' => 'Which one',
        'remove_which_helper' => 'Only the languages somebody uploaded are listed. The ones this plugin carries arrive with every release and are not anybody’s to remove.',
        'remove_confirm' => 'Both halves go: this plugin’s strings and the panel’s. It cannot be undone from here, and the only copy left is whatever the person who made it still has. The copy kept off the panel is forgotten too, so a restore will not bring it back.',
        'remove_refused' => 'That language was left where it is',
        'remove_refused_bad_code' => 'That is not a name a language can have.',
        'remove_refused_shipped' => 'This plugin carries that language itself, so it arrives again with every release. There is nothing to remove.',
        'remove_refused_unknown' => 'Nothing was ever uploaded under that name.',
        'remove_refused_in_use' => 'It is the language the panel falls back to. Choose another main language first, then remove this one.',
        'remove_refused_stuck' => 'The files would not go. The panel could not write to its own language directory.',
        'removed' => ':code has been removed',
        'removed_body' => 'Anybody reading the panel in it now reads it in the main language instead.',
