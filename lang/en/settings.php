...
return [
    ...
    'groups' => [
        ...
        'features_helper' => 'Unticking one takes it out of the panel entirely. Its own settings are kept and its page keeps its address, so nothing is lost by switching something off to see what it was doing. Everything this plugin adds is in this list — the console button, the page arranger and per-person styles moved here from switches of their own, because a list that claims to hold every switch has to hold every switch. Most of these also have a permission under Roles, for handing one out without handing over the rest. Not all: the resource meters, the sidebar footer and the settings search are drawn for everybody and administered by nobody, the star on a server card belongs to whoever clicked it, and the Palworld and Minecraft pages inside a server go by that server own permissions rather than by one of these. The styling itself is not in this list — that has its own switch, under Look → Appearance → Style → None.',
    ...
    'pages' => [
        ...
        'capacity' => 'Capacity',
        'capacity_helper' => 'What has been promised on each machine against what it may hand out, so you can see whether another server fits. Pelican\'s node list shows a name and a count of servers, and the Machines block on the dashboard shows what is running - this is the third question, and the arithmetic is Pelican\'s own. Read only. Its own permission.',
        'schedules' => 'Schedules',
        'schedules_helper' => 'Every scheduled task on the panel with which of them have stopped: stuck part way through a run, overdue because the cron is not running, or never run at all. Pelican shows schedules inside each server and its own status has no word for any of those. Read only. Its own permission.',
        'activity' => 'Activity',
        'activity_helper' => 'Every event the panel logs, in one list instead of one server at a time. Pelican keeps the log and shows it per server; this asks the same log the other way round. Read only. Its own permission, because a record of who did what is a thing to hand over deliberately.',
        ...
        'api' => 'API',
        'api_helper' => 'The keys people hold, who has asked for one, and what each of them may see.',
        'languages' => 'Languages',
    ...
    'features' => [
        ...
        'console_helper' => 'The floating button inside a server, with the console and the power buttons on it, reaching the node directly. What shape it takes is on the server pages settings; this decides whether it is drawn at all.',
        'arranger' => 'Page arranger',
        'arranger_helper' => 'Dragging the blocks on a page into the order somebody wants. It carries its own permission under Roles, so this decides whether the panel offers it and the permission decides to whom.',
        'user_themes' => 'Per-person styles',
        'user_themes_helper' => 'Letting each person pick a style from the ones you offer, under Appearance in the client area. Which styles are offered is on the Look page; this decides whether anybody is asked at all.',
        'api' => 'API',
        'api_helper' => 'A way in from outside the panel: an address a Discord bot or a script of your own can ask for what this plugin knows — who is playing, which servers have no backup, whether another one fits on a node. Off registers no route at all rather than one that refuses, which is less surface rather than a politer amount of it. Anybody signed in may ask for a key that answers only for their own servers; granting one, refusing one, revoking one somebody else holds and issuing a panel-wide one all need the permission.',
        'languages' => 'Languages',
        ...
        'my_backups' => 'Warning on the server list',
        'my_backups_helper' => 'A line above each person\'s own server list when something of theirs needs attention: a server that has never been backed up, one that has gone stale, or a scheduled task that has stopped. Pelican\'s cards say what a server is doing now; nothing there says a backup has not run in three weeks, or that the schedule which was supposed to make one died in August. Drawn only when something is behind, and it names no server they could not already open.',
        ...
        'schedules_helper' => 'The admin page listing every scheduled task across the panel, worst first - stuck, overdue, or never run. Read only; everything that edits or runs one stays on Pelican\'s own page for that server.',
        'activity' => 'Panel activity',
        'activity_helper' => 'The admin page listing every logged event across the panel, newest first, with who did it and on which server. Read only - it deletes nothing, and Pelican\'s own setting still decides how long lines are kept.',
        ...
        'shop' => 'Shop',
        'shop_helper' => 'Selling servers from the panel: the store and checkout in the client area, each person\'s billing page, and the Shop settings page for the currency, the tax and the wording. The master switch — off, nobody can buy or pay, and what was already sold is still administered through the pages below.',
        'packages' => 'Packages',
        'packages_helper' => 'The admin page where what is for sale is defined: a server template with a price, a period and a stock. Its own permission, because setting prices is a different job from marking invoices paid.',
        'orders' => 'Orders',
        'orders_helper' => 'The admin page listing everything that was bought, the server each one became, and its standing — pending, active, suspended, cancelled. Its own permission.',
        'invoices' => 'Invoices',
        'invoices_helper' => 'The admin page listing what is owed and what was paid, with a button to mark an invoice paid by hand. Its own permission, because that button is where money is recorded.',
        'payments' => 'Payments',
        ...
        'overview_helper' => 'The page that answers what came in this month, what is owed, what the live services are worth every month and what needs looking at today. Its own right, because turnover is not something everybody who may price a package should be able to read.',
