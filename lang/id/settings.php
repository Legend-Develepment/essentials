...
return [
    ...
    'groups' => [
        ...
        'valheim' => 'Valheim',
        'languages' => 'Bahasa',
        'files_where' => 'Di mana berkas disimpan',
        'files_bucket' => 'Bucket-nya',
        'files_cdn' => 'CDN-nya',
        'files_mirror' => 'Bahasa, disimpan di luar panel',
    ...
    /*
     * Di mana berkas yang disimpan plugin ini ditaruh.
     *
     * Kata-katanya berbicara tentang tujuan, bukan tentang penyedia, karena tiga
     * kalimat yang sama berlaku untuk sebuah bucket maupun sebuah CDN, dan
     * administrator yang menyiapkan salah satunya tidak peduli sedang melihat
     * yang mana sampai kolomnya berbeda.
     */
    'files' => [
        'where' => 'Berkas disimpan',
        'where_helper' => 'Di panel, berkas berada di disk panel sendiri, tempat mereka selama ini ditaruh dan tidak butuh persiapan apa pun. Di tempat lain berarti di suatu tempat yang tidak harus dipegang panel ini, dan disajikan lebih dekat ke siapa pun yang melihatnya. Tujuan yang tidak mau menjawab jatuh kembali ke panel alih-alih kehilangan unggahan.',
        'panel' => 'Di panel ini',
        's3' => 'Di sebuah bucket (S3, R2, MinIO, Wasabi)',
        'cdn' => 'Di sebuah CDN',
        'read_from' => 'Dibaca dari',
        'read_from_helper' => 'Dari mana sebuah berkas diambil, yang tidak selalu sama dengan tempat ia ditulis. CDN di depan sebuah bucket ditulis di sini, begitu juga alamat pengiriman yang berbeda dari alamat API-nya. Kosong berarti tujuannya sendiri yang menentukan.',
        ...
        'cdn_title' => 'CDN-nya',
        'cdn_helper' => 'Sebuah CDN yang berbicara dengan API Modora. Token-nya adalah token server ke server dan ia admin penuh atas akun itu, jadi ia dijauhkan dari berkas pengaturan yang diekspor sama seperti setiap kredensial lain di sini.',
        'cdn_base' => 'Alamat',
        'cdn_base_helper' => 'Di mana API-nya berada. Kalau berkas disajikan dari tempat lain, tulis alamat itu di Dibaca dari, di atas.',
        'cdn_token' => 'Token',
        'cdn_token_helper' => 'Dikirim sebagai X-Internal-Token. Siapa pun yang memegangnya bisa menulis ke dan menghapus dari seluruh akun.',
        'cdn_folder' => 'Folder',
        'cdn_folder_helper' => 'Sebuah folder di bawah akun untuk menyimpan berkas panel ini, supaya satu CDN bisa melayani beberapa panel tanpa saling menginjak.',

        'move' => 'Pindahkan yang masih ada di panel',
        'move_confirm' => 'Ikon bilah samping, latar panel dan latar layar masuk disalin ke tujuan dan alamatnya ditulis ulang. Salinan di panel ini dibiarkan di tempatnya, jadi tidak ada yang rusak kalau kamu berubah pikiran. Gambar yang diunggah mulai sekarang tetap pergi ke tujuan; ini hanya untuk yang sudah ada di sini.',
        'move_done' => 'Dipindahkan',
        'move_done_body' => 'Dilihat :looked, dipindahkan :moved, tidak bisa dipindahkan :failed.',
        'check' => 'Uji ini',
        'check_ok' => 'Ini bekerja',
        'check_ok_body' => 'Sebuah berkas ditulis, diambil kembali lewat alamat publiknya dan dihapus lagi.',
        'check_bad' => 'Itu tidak bekerja',
        'check_panel' => 'Berkas disetel untuk disimpan di panel ini, jadi tidak ada yang bisa diuji.',
        'check_refused' => 'Tujuannya menolak berkasnya dan tidak mengatakan kenapa.',
        'check_unreadable' => 'Ia menerima berkasnya, tapi berkasnya tidak bisa dibaca kembali dari :url. Itu alamat yang akan dipakai peramban, jadi berkas yang tidak bisa diambil siapa pun nanti menjadi gambar yang rusak. Periksa Dibaca dari, dan apakah tujuannya menyajikan berkas secara publik.',
        'bucket_missing' => 'Access key, secret dan nama bucket ketiganya dibutuhkan sebelum ada yang bisa diuji.',
        'cdn_missing' => 'Alamat dan token keduanya dibutuhkan sebelum ada yang bisa diuji.',
        'cdn_shape' => 'Ia menerima berkasnya lalu menjawab dalam bentuk yang tidak bisa ditemukan alamatnya oleh panel ini. Yang dikatakannya: :body',
        'mirror_minutes' => 'Cari bahasa yang berubah setiap',
        'mirror_minutes_helper' => 'Dalam menit. Mencari itu murah: setiap bahasa yang diunggah dibaca, di-hash dan dibandingkan dengan yang terakhir dikirim, jadi sapuan biasa tidak mengirim apa pun. Hanya bahasa yang diubah seseorang yang menyeberang.',
        'mirror_now' => 'Salin bahasa sekarang',
        'mirror_done' => 'Bahasa disalin',
        'mirror_done_body' => 'Dilihat :looked, dikirim :sent, tidak bisa dikirim :failed.',
        'mirror_restore' => 'Pulihkan bahasa',
        'mirror_restore_confirm' => 'Ini menuliskan setiap bahasa dari salinan di luar panel menimpa apa yang ada di panel ini. Itulah gunanya setelah sebuah pembaruan, dan sesudahnya tidak ada cara menghapus bahasa yang sudah terpasang, jadi sebaiknya kamu yakin dulu.',
        'mirror_back' => 'Bahasa dipulihkan',
        'mirror_back_body' => 'Ditemukan :found, dikembalikan :put, tidak bisa diambil :failed.',
    ],
    ...
    'pages' => [
        ...
        'files' => 'Penyimpanan dan CDN',
        'files_helper' => 'Di mana berkas yang disimpan plugin ini ditaruh, dan dari alamat mana ia dibaca.',
    ],
    ...
    'features' => [
        ...
        'files' => 'Penyimpanan dan CDN',
        'files_helper' => 'Menyimpan berkas plugin ini di tempat selain panel: sebuah bucket S3, atau sebuah CDN. Mati bukan berarti "tanpa berkas" - berarti disk panel itu sendiri, tempat berkas selama ini ditaruh. Yang ditentukan di sini adalah apakah tempat lain ditawarkan sama sekali. Tujuan yang tidak mau menjawab jatuh kembali ke panel alih-alih kehilangan unggahan, dan alamat yang sudah ditulis tidak pernah ditarik kembali: mengubah ini menentukan ke mana berkas berikutnya pergi, bukan di mana berkas terakhir berada.',
        'minecraft' => 'Minecraft',
        ...
        'updating' => 'Pemberitahuan pembaruan',
        'waitlist' => 'Daftar tunggu',
        'waitlist_helper' => 'Memungkinkan seseorang minta dikabari kalau paket yang habis terjual dijual lagi. Begitu stoknya kembali, semua yang menunggu paket itu dikabari sekaligus dan barangnya jatuh ke siapa pun yang membeli lebih dulu - tidak ada yang ditahan untuk siapa pun, dan setiap pesan mengatakan itu. Dikabari berarti dicoret dari daftar, jadi satu permintaan dibayar dengan satu pemberitahuan, bukan langganan yang terus berjalan. Ini butuh toko, dan inilah satu-satunya bagian toko yang menulis kepada pelanggan yang belum membeli apa pun.',
        'updating_helper' => 'Satu baris di atas halaman selagi plugin ini memasang pembaruan, dan sampai lima menit setelah selesai. Ia tidak bisa ditampilkan selama pembaruannya sendiri berlangsung - selagi rilisnya ditukar, Pelican membaca plugin ini sebagai tidak terpasang dan tidak memuat apa pun darinya, jadi tidak ada milik kita yang tersisa untuk menggambar. Gunanya untuk orang yang menemui halaman setengah jadi, menunggu, lalu kembali: baris itu memberitahunya apa yang tadi ia lihat.',
        ...
        'credit' => 'Saldo dan pengembalian dana',
        'credit_helper' => 'Uang yang dipegang toko untuk seorang pelanggan. Pengembalian dana bisa kembali ke kartu asalnya atau tinggal di akun sebagai saldo; bagaimanapun caranya sebuah nota kredit tetap ditulis, dan saldo di sebuah akun otomatis dipotong dari faktur berikutnya sebelum pelanggan pernah diminta membayar. Hak tersendiri, karena menandai sebuah faktur lunas mencatat bahwa uang masuk sedangkan ini mengeluarkan uang.',
        'upgrades' => 'Naik dan turun paket',
        'upgrades_helper' => 'Memindahkan layanan yang berjalan ke paket lain tanpa membeli yang baru. Sisa periode yang sudah dibayar kembali, rentang yang sama dihitung dengan harga baru, dan selisihnya ditagihkan atau ditaruh di akun pelanggan. Setiap paket menyebut ke paket mana saja ia boleh dipindahkan, dan hanya yang berbagi egg yang sama yang ditawarkan: egg yang lain berarti server yang lain, bukan yang lebih besar.',
        'addons' => 'Tambahan',
        'addons_helper' => 'Barang yang dijual berdampingan dengan sebuah paket: memori lebih banyak, satu slot cadangan lagi, atau sesuatu yang hanya berupa satu baris di faktur. Masing-masing menyebut cocok dengan paket mana dan apa yang ia tambahkan ke server, dan ia ditagih entah pada setiap perpanjangan atau sekali saja. Dibeli saat memesan atau belakangan pada layanan yang sedang berjalan, dan di sana harganya dihitung menurut sisa periode. Hak tersendiri, karena apa yang boleh ditambahkan sebuah tambahan ke server seseorang adalah keputusan tentang mesin mereka, bukan tentang daftar harga.',
        'tickets' => 'Tiket',
        'tickets_helper' => 'Tempat pelanggan bertanya dari dalam panel, di samping layanan yang mereka tanyakan - dan justru itulah yang tidak bisa dilakukan sebuah kanal obrolan. Dijawab di sebuah halaman di sini, atau diserahkan ke Discord lewat Modora, tergantung bagaimana halaman Tiket disetel. Setiap pertanyaan dan setiap jawaban tetap disimpan di panel ini bagaimanapun caranya, jadi tidak ada yang hilang ketika ujung sana tidak bisa dijangkau. Hak tersendiri, karena menjawab pelanggan adalah pekerjaan yang diberikan kepada seseorang, bukan yang datang bersama menentukan harga paket.',
    ...
    'channel' => [
        ...
        'token' => 'Token repositori dev',
        'token_helper' => 'Saluran dev diterbitkan dari sebuah repositori privat, jadi membacanya butuh token GitHub - sebuah fine-grained personal access token dengan akses baca ke isi repositori itu, dan tidak lebih. Stabil dan beta bersifat publik dan tidak butuh apa pun. Token itu tinggal di panel ini: ia tidak ditulis ke berkas pengaturan yang diekspor.',
        'stable' => 'Stabil',
        'beta' => 'Beta',
