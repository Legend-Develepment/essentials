...
return [
    ...
    'groups' => [
        ...
        'valheim' => 'Valheim',
        'languages' => 'Ngôn ngữ',
        'files_where' => 'Nơi giữ tệp',
        'files_bucket' => 'Bucket',
        'files_cdn' => 'CDN',
        'files_mirror' => 'Ngôn ngữ, giữ ngoài bảng điều khiển',
    ...
    /*
     * Nơi đặt những tệp mà plugin này giữ.
     *
     * Chữ ở đây nói về một đích đến chứ không nói về một nhà cung cấp, bởi vì
     * cùng ba câu ấy đúng với một bucket và đúng với một CDN, mà người quản trị
     * đang dựng một cái thì chẳng bận tâm họ đang nhìn cái nào cho tới lúc các
     * ô nhập khác nhau.
     */
    'files' => [
        'where' => 'Tệp được giữ ở',
        'where_helper' => 'Trên bảng điều khiển thì chúng nằm trên chính ổ đĩa của nó, chỗ xưa nay chúng vẫn đi tới và chẳng phải dựng gì cả. Chỗ nào khác là chỗ mà bảng điều khiển này không phải ôm chúng, và được phục vụ gần hơn với người đang xem. Một đích đến không chịu trả lời thì rơi về bảng điều khiển chứ không làm mất một lần tải lên.',
        'panel' => 'Trên bảng điều khiển này',
        's3' => 'Trong một bucket (S3, R2, MinIO, Wasabi)',
        'cdn' => 'Trên một CDN',
        'read_from' => 'Đọc từ',
        'read_from_helper' => 'Một tệp được lấy về từ đâu, mà chỗ đó không phải lúc nào cũng là chỗ nó được ghi vào. Một CDN đứng trước một bucket thì điền vào đây, và một địa chỉ phân phối khác với địa chỉ đặt API cũng vậy. Để trống thì đích đến tự lo lấy.',

        'bucket' => 'Bucket',
        'bucket_helper' => 'Bất cứ thứ gì nói giao thức S3. Ô endpoint và công tắc path-style là thứ mà những cái không phải AWS cần; với chính AWS thì cứ để yên cả hai.',
        'bucket_key' => 'Khóa truy cập',
        'bucket_secret' => 'Khóa bí mật',
        'bucket_name' => 'Tên bucket',
        'bucket_region' => 'Vùng',
        'bucket_region_helper' => 'auto hợp với R2 và hầu hết bản tự dựng. AWS muốn vùng của riêng nó, như eu-central-1.',
        'bucket_endpoint' => 'Endpoint',
        'bucket_endpoint_helper' => 'Để trống với AWS. R2, MinIO và những cái còn lại đều có endpoint riêng.',
        'bucket_path_style' => 'Địa chỉ kiểu path-style',
        'bucket_path_style_helper' => 'Thứ MinIO và hầu hết bản tự dựng cần. AWS và R2 thì không.',

        'cdn_title' => 'CDN',
        'cdn_helper' => 'Một CDN nói API của Modora. Token là loại máy nói với máy và nó là quyền quản trị đầy đủ trên tài khoản đó, nên nó được giữ ngoài tệp cài đặt xuất ra, y như mọi thông tin đăng nhập khác ở đây.',
        'cdn_base' => 'Địa chỉ',
        'cdn_base_helper' => 'Nơi đặt API. Nếu tệp được phục vụ từ chỗ khác, hãy điền địa chỉ đó vào ô Đọc từ ở trên.',
        'cdn_token' => 'Token',
        'cdn_token_helper' => 'Được gửi trong X-Internal-Token. Ai giữ nó cũng ghi và xóa được trên cả tài khoản.',
        'cdn_folder' => 'Thư mục',
        'cdn_folder_helper' => 'Một thư mục nằm trong tài khoản để giữ tệp của bảng điều khiển này, để một CDN phục vụ được nhiều bảng điều khiển mà chúng không giẫm lên nhau.',

        'move' => 'Chuyển những gì còn nằm trên bảng điều khiển',
        'move_confirm' => 'Biểu tượng thanh bên, ảnh nền bảng điều khiển và ảnh nền màn hình đăng nhập được chép sang đích đến và địa chỉ của chúng được viết lại. Bản chép trên bảng điều khiển này vẫn nằm nguyên chỗ cũ, nên chẳng có gì hỏng nếu bạn đổi ý. Ảnh tải lên từ đây trở đi thì đằng nào cũng đi tới đích đến; cái này chỉ dành cho những ảnh đã có sẵn ở đây.',
        'move_done' => 'Đã chuyển',
        'move_done_body' => 'Đã xem :looked, đã chuyển :moved, không chuyển được :failed.',
        'check' => 'Thử cái này',
        'check_ok' => 'Nó chạy được',
        'check_ok_body' => 'Một tệp đã được ghi, lấy lại qua địa chỉ công khai của nó, rồi xóa đi.',
        'check_bad' => 'Việc đó không chạy',
        'check_panel' => 'Tệp đang được đặt là giữ trên bảng điều khiển này, nên không có gì để thử.',
        'check_refused' => 'Đích đến từ chối tệp và không nói gì về lý do.',
        'check_unreadable' => 'Nó nhận tệp, nhưng không đọc lại được từ :url. Đó là địa chỉ mà trình duyệt sẽ dùng, nên một tệp không ai lấy được về sau sẽ là một bức ảnh hỏng. Hãy kiểm tra ô Đọc từ, và xem đích đến có phục vụ tệp công khai không.',
        'bucket_missing' => 'Cần đủ cả khóa truy cập, khóa bí mật và tên bucket thì mới có gì để thử.',
        'cdn_missing' => 'Cần cả địa chỉ lẫn token thì mới có gì để thử.',
        'cdn_shape' => 'Nó nhận tệp rồi trả lời theo một hình dạng mà bảng điều khiển này không tìm ra địa chỉ nào trong đó. Nó nói thế này: :body',
        'mirror_minutes' => 'Tìm ngôn ngữ đã đổi mỗi',
        'mirror_minutes_helper' => 'Tính bằng phút. Việc tìm rất rẻ: mỗi ngôn ngữ đã tải lên đều được đọc, băm, rồi so với thứ đã gửi lần cuối, nên một vòng bình thường chẳng gửi gì cả. Chỉ ngôn ngữ nào có người sửa mới đi qua đường truyền.',
        'mirror_now' => 'Chép ngôn ngữ ngay',
        'mirror_done' => 'Đã chép ngôn ngữ',
        'mirror_done_body' => 'Đã xem :looked, đã gửi :sent, không gửi được :failed.',
        'mirror_restore' => 'Khôi phục ngôn ngữ',
        'mirror_restore_confirm' => 'Việc này ghi mọi ngôn ngữ trong bản chép ngoài bảng điều khiển đè lên những gì đang có trên bảng điều khiển này. Đó đúng là công dụng của nó sau một lần nâng cấp, và sau đó không có cách nào gỡ một ngôn ngữ đã cài, nên chắc chắn rồi hãy làm.',
        'mirror_back' => 'Đã khôi phục ngôn ngữ',
        'mirror_back_body' => 'Tìm thấy :found, đặt lại :put, không lấy được :failed.',
    ],
    ...
    'pages' => [
        ...
        'files' => 'Lưu trữ và CDN',
        'files_helper' => 'Những tệp plugin này giữ được đặt ở đâu, và chúng được đọc từ địa chỉ nào.',
    ],
    ...
    'features' => [
        ...
        'console' => 'Nút bảng điều khiển lệnh',
        'console_helper' => 'Cái nút nổi bên trong một máy chủ, với bảng điều khiển lệnh và các nút nguồn trên đó, với thẳng tới node. Nó mang hình dáng nào là việc của phần cài đặt các trang trong máy chủ; cái này quyết định có vẽ nó ra hay không.',
        'arranger' => 'Trình sắp xếp trang',
        'arranger_helper' => 'Kéo các khối trên một trang vào đúng thứ tự ai đó muốn. Nó mang một quyền riêng ở trang vai trò, nên cái này quyết định bảng điều khiển có mời dùng hay không, còn cái quyền kia quyết định mời ai.',
        'user_themes' => 'Kiểu dáng riêng từng người',
        'user_themes_helper' => 'Cho từng người tự chọn một kiểu dáng trong số bạn đưa ra, ở mục Diện mạo trong khu vực khách hàng. Đưa ra những kiểu dáng nào là việc của trang Diện mạo; cái này quyết định có ai được hỏi hay không.',
        'api' => 'API',
        ...
        'files' => 'Lưu trữ và CDN',
        'files_helper' => 'Giữ tệp của plugin này ở chỗ nào đó ngoài bảng điều khiển: một bucket S3, hoặc một CDN. Tắt không có nghĩa là "không có tệp" - nó có nghĩa là ổ đĩa của chính bảng điều khiển, chỗ xưa nay chúng vẫn đi tới. Cái này quyết định xem có chỗ nào khác được đưa ra hay không. Một đích đến không chịu trả lời thì rơi về bảng điều khiển chứ không làm mất một lần tải lên, và một địa chỉ đã được ghi xuống thì không bao giờ bị lấy lại: đổi cái này là quyết định tệp kế tiếp đi đâu, chứ không phải tệp trước đó nằm ở đâu.',
        'minecraft' => 'Minecraft',
        ...
        'updating' => 'Thông báo đang cập nhật',
        'waitlist' => 'Danh sách chờ',
        'waitlist_helper' => 'Cho phép một người xin được báo tin khi một gói đã bán hết lại có hàng. Khi hàng về, mọi người đang chờ gói ấy được báo cùng một lúc và nó thuộc về người mua trước - không có gì được giữ chỗ cho ai cả, và mọi tin nhắn đều nói vậy. Được báo tin là bị đưa ra khỏi danh sách, nên một lần xin đổi lấy một lần báo chứ không bao giờ là một đăng ký dài hạn. Nó cần có cửa hàng, và nó là thứ duy nhất trong cửa hàng viết thư cho một người khách chưa mua gì.',
        'updating_helper' => 'Một dòng ở đầu trang trong lúc plugin này cài một bản cập nhật, và trong năm phút sau khi nó xong. Nó không hiện ra được trong chính lúc cập nhật - khi bản phát hành đang được thay vào, Pelican đọc plugin này là chưa cài và không nạp gì của nó cả, nên chẳng còn gì của chúng ta để vẽ lên. Nó dành cho người gặp một trang vẽ dở, chờ, rồi quay lại: dòng ấy nói cho họ biết họ đã thấy cái gì.',
        ...
        'shop' => 'Cửa hàng',
        'shop_helper' => 'Bán máy chủ ngay từ bảng điều khiển: cửa hàng và trang thanh toán ở phía khách, trang hóa đơn của mỗi người, và trang Cài đặt cửa hàng cho đơn vị tiền, thuế và các đoạn chữ. Công tắc chính - tắt thì không ai mua hay trả tiền được, còn những gì đã bán vẫn được quản lý qua các trang bên dưới.',
        'packages' => 'Gói',
        'packages_helper' => 'Trang quản trị nơi quyết định bán gì: một khuôn mẫu máy chủ kèm giá, kỳ hạn và tồn kho. Quyền riêng, vì đặt giá là việc khác với việc đánh dấu hóa đơn đã trả.',
        'orders' => 'Đơn hàng',
        'orders_helper' => 'Trang quản trị liệt kê mọi thứ đã mua, máy chủ mà mỗi đơn hàng trở thành, và trạng thái của nó - chờ, đang chạy, bị dừng, đã hủy. Quyền riêng.',
        'invoices' => 'Hóa đơn',
        'invoices_helper' => 'Trang quản trị cho biết khoản nào còn nợ và khoản nào đã trả, kèm nút đánh dấu một hóa đơn là đã trả bằng tay. Quyền riêng, vì nút đó chính là chỗ tiền được ghi sổ.',
        'payments' => 'Thanh toán',
        ...
        'credit' => 'Số dư và hoàn tiền',
        'credit_helper' => 'Tiền cửa hàng giữ hộ một người khách. Một lần hoàn tiền có thể về lại thẻ đã trả hoặc nằm lại trong tài khoản dưới dạng số dư; đằng nào cũng có một phiếu ghi có được viết, và số dư trong tài khoản tự trừ vào hóa đơn kế tiếp trước cả khi có ai hỏi khách trả tiền. Có quyền riêng, bởi vì đánh dấu một hóa đơn đã trả là ghi lại rằng tiền đã tới, còn cái này thì phát tiền ra.',
        'upgrades' => 'Nâng cấp và hạ cấp',
        'upgrades_helper' => 'Chuyển một dịch vụ đang chạy sang gói khác mà không phải mua mới. Phần còn lại của kỳ đã trả tiền được trả lại, đúng khoảng thời gian ấy được tính theo giá mới, và phần chênh lệch thì ra hóa đơn hoặc vào tài khoản của khách. Mỗi gói liệt kê những gói nào nó được chuyển sang, và chỉ những gói dùng chung egg mới được đưa ra: một egg khác là một máy chủ khác, không phải một máy chủ to hơn.',
        'addons' => 'Tiện ích thêm',
        'addons_helper' => 'Những thứ bán kèm một gói: thêm bộ nhớ, thêm một chỗ sao lưu, hoặc một thứ chỉ là một dòng trên hóa đơn. Mỗi cái nói rõ nó hợp với những gói nào và nó thêm gì cho máy chủ, và nó được tính tiền hoặc theo mỗi lần gia hạn hoặc một lần duy nhất. Mua lúc thanh toán, hoặc mua sau trên một dịch vụ đang chạy, khi ấy nó được tính theo phần còn lại của kỳ. Có quyền riêng, bởi vì một tiện ích được phép thêm gì vào máy chủ của người ta là một quyết định về cái máy của họ chứ không phải về một bảng giá.',
        'tickets' => 'Yêu cầu hỗ trợ',
        'tickets_helper' => 'Một chỗ để khách hỏi ngay từ trong bảng điều khiển, ngay cạnh cái dịch vụ họ đang hỏi về - và đó là điều duy nhất một kênh chat không làm được. Trả lời trên một trang ở đây, hoặc giao cho Discord qua Modora, tùy trang Yêu cầu hỗ trợ được đặt thế nào. Đằng nào thì mọi câu hỏi và mọi câu trả lời cũng được giữ trong chính bảng điều khiển này, nên không mất gì khi đầu bên kia không với tới được. Có quyền riêng, bởi vì trả lời khách là một việc được giao cho ai đó chứ không phải việc đi kèm với chuyện đặt giá cho gói.',
        'overview' => 'Tổng quan cửa hàng',
        'overview_helper' => 'Trang trả lời tháng này đã vào bao nhiêu, còn nợ bao nhiêu, các dịch vụ đang chạy đáng bao nhiêu mỗi tháng, và hôm nay cần để mắt tới cái gì. Có quyền riêng, bởi vì doanh thu không phải thứ mà ai được đặt giá cho một gói cũng nên đọc được.',
        'terminate' => 'Chấm dứt một dịch vụ',
        'terminate_helper' => 'Cái nút dừng một dịch vụ ngay và xóa máy chủ của nó, cả tập tin lẫn mọi thứ. Tách khỏi quyền đơn hàng là có chủ ý: dừng, dời kỳ hạn và hủy đều lùi lại được, còn cái này thì không. Người trả lời phiếu hỗ trợ có thể có ba cái đầu mà không có cái này.',
    ...
    'channel' => [
        ...
        'token' => 'Token của repository dev',
        'token_helper' => 'Kênh dev được phát hành từ một repository riêng tư, nên muốn đọc nó thì cần một token GitHub - một fine-grained personal access token có quyền đọc nội dung của repository đó, và không gì hơn. Bản ổn định và beta thì công khai, không cần token nào. Nó ở lại trên bảng điều khiển này: nó không được ghi vào tệp cài đặt xuất ra.',
        'stable' => 'Ổn định',
        'beta' => 'Beta',
