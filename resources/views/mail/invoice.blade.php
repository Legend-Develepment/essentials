...
<div style="font: 15px/1.6 system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif; color: #16181d; max-width: 34rem;">
    ...
    {{-- Who sent it, under a rule. A mail carrying an invoice should say who
         the invoice is from without the reader having to open the document -
         and the address and the numbers are what makes it answerable. --}}
    @php($issuer = \LegendDevelopment\Theme\Support\Shop\Billing::issuer())

    <p style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid #e3e6ea; color: #5c636b; font-size: 0.8125rem; line-height: 1.5;">
        <strong style="color: #16181d;">{{ $issuer['name'] }}</strong><br>
        ...
        @if ($issuer['vat'] !== '')
            {{ \LegendDevelopment\Theme\Support\Theme::trans('invoices.doc_vat') }} {{ $issuer['vat'] }}<br>
        @endif

        @if ($issuer['coc'] !== '')
            {{ \LegendDevelopment\Theme\Support\Theme::trans('invoices.doc_coc') }} {{ $issuer['coc'] }}
        @endif
    </p>
</div>
